...
The implementation emits explicit `area_added` / `area_removed` raw events keyed by canonical session identity plus `area_id`, including deterministic ordering and GUID-less session-map fallback attribution. Area events remain orthogonal to nested object additions/removals and carry no gameplay-semantic interpretation. Issue #58 makes the evidence boundary explicit: canonical v1 retains only the player-area projection, so an `area_added` / `area_removed` event establishes only entry into or exit from that projection; it does not uniquely establish physical area creation/destruction, a specific ownership transition, settlement, conquest, or another target-side cause that can produce the same canonical transition. Synthetic regression coverage and cross-platform PR CI establish the implemented projection-event behavior. The motivating private transitions have not been independently re-checked as target evidence, and broader parse-degradation/ownership-recognition audit findings remain separate; therefore target validation is still incomplete.
