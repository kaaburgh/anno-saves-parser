...
The four-worker measurements reused the same two private saves under distinct local names to stress resource scaling; they are throughput/resource evidence only, not four independent semantic fixtures. A separate 1–64 KiB sweep found a broad fast plateau rather than a unique optimum. The existing evidence therefore supports 16 KiB as a conservative implementation default because it captures the observed memory reduction without demonstrating a throughput advantage for retaining 1 MiB.

Production `anno_save_probe.zlib_to_file()` now uses the named internal `DECOMPRESSION_CHUNK_BYTES = 16 << 10` input step. The retained `decompression.py` helper and differential oracle continue to compare that behavior with the historical 1 MiB reference, while `decompression_target_check.py` and `decompression_resource_check.py` provide bounded operator-side canonical-equality, timing, and workers=1/2 resource procedures over verified temporary source snapshots. These merged tools and synthetic regressions establish the production contract and reproducible target-validation procedure; they do not by themselves establish new real-target resource measurements.

Issue #34 remains open pending the representative private-save validation required by its acceptance criteria: CPython workers 1/2 wall time and peak memory, informative PyPy measurements where available, and conservative Windows real-save evidence. No public chunk-size tuning flag is justified by the current evidence, and exact resource numbers remain local evidence rather than a portable performance contract until those runs are accepted.
