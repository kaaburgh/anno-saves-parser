...
The repository now contains the bounded implementation prerequisites for #34 without changing the production default: `decompression.py` defines the named 16 KiB candidate and is covered by a differential oracle against both the historical 1 MiB reference and the current production decompressor; `decompression_target_check.py` provides the operator-side whole-parser canonical-equality/wall-time harness with balanced paired timing order, read-only source handling, and verified temporary snapshots. Before repeated canonicalization, each source save is hashed and sized before copying, the temporary snapshot is hashed and sized, and the live source is hashed and sized again; any mismatch fails closed. All repeated 1 MiB/16 KiB runs then parse only that verified snapshot. These merged helpers establish synthetic capability and a reproducible target-validation procedure, not real target validation by themselves.

Issue #34 remains open because production `anno_save_probe.zlib_to_file()` still uses the 1 MiB compressed-input step. Completion still requires wiring the production path to the 16 KiB default while preserving streaming, progress and cleanup behavior, then running the representative private-save comparison required by the issue for CPython workers 1/2 (with PyPy informative where available), recording peak memory as PSS/RSS where practical, and reporting Windows real-save evidence conservatively. No public chunk-size tuning flag is justified by the current evidence.
